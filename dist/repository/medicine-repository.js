...
class MedicineRepository {
  ...
  async updateMedicine(id, updatedData) {
    try {
      const updatedMedicine = await Medicine.findByIdAndUpdate(id, updatedData, {
        new: true
      });
  ...
  async getMedicinesByManufacturer(manufacturer) {
    try {
      ...
      console.error('Error in getMedicinesByManufacturer repository method:', error);
      throw {
        error
      };
    }
  ...
  async updateDiscountByManufacturer(manufacturer, discount) {
    try {
      const previousMedicines = await Medicine.find({
        MANUFACTURER: manufacturer
      });
